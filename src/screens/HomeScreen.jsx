...
export default function HomeScreen() {
  ...
  // ─── FIX 1: Smoother spring config — no overshootClamping, softer stiffness ───
  const animateDashboard = (toValue, velocity = 0, onFinish = null) => {
    Animated.spring(dashboardAnim, {
      toValue,
      velocity,
      useNativeDriver: true,
      stiffness: 260,        // ↓ was 350 — softer, more natural feel
      damping: 32,           // slightly softer
      mass: 0.9,
      overshootClamping: false, // ← FIX: was true, caused abrupt mid-swipe stops
  ...
  const toggleActivities = useCallback(() => {
    setShowActivities(!showActivitiesRef.current);
  }, []);

  const gestureStartPosRef = useRef(0);
  const currentDashboardPos = useRef(0);

  useEffect(() => {
    const listenerId = dashboardAnim.addListener(({ value }) => {
      currentDashboardPos.current = value;
    });
    return () => dashboardAnim.removeListener(listenerId);
  }, [dashboardAnim]);
  console.log("currentDashboard");

  // ─── FIX 2: Single panResponder, no dual-spread ───────────────────────────────
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (evt, gestureState) => {
        // Use non-capture so ScrollView can still claim if needed
        ...
        if (!currentShow) {
          return isScrollAtTop.current && dy > 8;
        } else {
          return dy < -5;
        }
      },
      onPanResponderGrant: () => {
        // ─── FIX 3: Stop animation synchronously, capture current pos ───────────
        dashboardAnim.stopAnimation();
        gestureStartPosRef.current = currentDashboardPos.current;
      },
      onPanResponderMove: (_, gestureState) => {
        const windowHeight = Dimensions.get('window').height;
        const revealDistance = windowHeight * 0.70;

        let newValue = gestureStartPosRef.current + gestureState.dy;
        if (newValue < 0) newValue = 0;
        if (newValue > revealDistance) newValue = revealDistance;
      ...
      onPanResponderRelease: (_, gestureState) => {
        ...
        if (isShortTap) {
          if (gestureState.y0 < 350) {
            setShowActivities(!showActivitiesRef.current);
            return;
          }
        }

        // ─── FIX 4: Use sync currentDashboardPos instead of async stopAnimation callback ───
        const currentVal = currentDashboardPos.current;
        const velocityThreshold = 0.15;
        const posThreshold = revealDistance * 0.3;

        const isFlingUp = gestureState.vy < -velocityThreshold;
        const isFlingDown = gestureState.vy > velocityThreshold;
        const isOpeningIntent = gestureState.dy > 50;
        const isClosingIntent = gestureState.dy < -50;

        if (isFlingUp || isClosingIntent) {
          setShowActivities(false, gestureState.vy);
        } else if (isFlingDown || isOpeningIntent || currentVal > posThreshold) {
          setShowActivities(true, gestureState.vy);
        } else {
          setShowActivities(false, gestureState.vy);
        }
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'home':
        return (
          <View style={{ flex: 1 }}>
            {/* Background Layer: Recent Activities */}
            <RecentActivitiesSheet
              visible={showActivities}
              animatedValue={dashboardAnim}
              onClose={() => setShowActivities(false)}
            ...
            {/* Foreground Layer */}
            <View
              style={[styles.foreground, { zIndex: 100 }]}
              pointerEvents="box-none"
            >
              {/* ─── FIX 5: panHandlers on ONE view only ─────────────────────────── */}
              <Animated.View
                style={[
                  StyleSheet.absoluteFill,
                  {
                    opacity: dashboardAnim.interpolate({
                      inputRange: [0, Dimensions.get('window').height * 0.3],
                      outputRange: [1, 0],
                      extrapolate: 'clamp',
                    }),
                    transform: [{
              ...
              {/* Single Animated.View with panHandlers — no more double-spread */}
              <Animated.View
                style={{
                  flex: 1,
                  transform: [{ translateY: dashboardAnim }],
                ...
                pointerEvents="box-none"
                {...panResponder.panHandlers}  // ← only here
              >
                <View style={{ flex: 1, backgroundColor: 'transparent' }}>
                  <View style={[styles.mainContentCard, { marginTop: paddingTop + 16 }]}>
                    ...
                    <Animated.ScrollView
                      ref={scrollRef}
                      style={{ flex: 1 }}
                      contentContainerStyle={[styles.scrollContent, { paddingTop: 490, flexGrow: 1 }]}
                      showsVerticalScrollIndicator={false}
                      bounces={true}
                      scrollEnabled={!showActivities}
                      refreshControl={
  ...
  return (
    <View style={styles.container}>
      ...
      <BottomNavBar
        activeTab={activeTab}
        onTabPress={setActiveTab}
        style={{ zIndex: 1000 }} // Ensure Navbar is on top of the dashboard (which is 100)
      />
    </View>
  );
}
